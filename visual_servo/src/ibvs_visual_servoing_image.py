...
while True:
    ...
    #Define the target positions in image coordinates. The target positions are the desired locations of each of the aruco markers corner points. 
    #top-left, top-right, bottom-right, and bottom-left order
    target_positions = np.array([(int((w/2)-20),int((h/2)-20)),(int((w/2)+20),int((h/2)-20)),(int((w/2)+20),int((h/2)+20)),(int((w/2)-20),int((h/2)+20))]).flatten()    
    ...
    markerSizeInCM = 5
    rvec , tvec, _  = aruco.estimatePoseSingleMarkers(corners, markerSizeInCM, mtx, None)
